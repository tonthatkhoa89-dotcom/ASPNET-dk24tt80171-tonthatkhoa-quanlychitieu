...
public class StatisticsHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string byCategorySql = @"SELECT c.CategoryName, SUM(tr.Amount) AS TotalAmount
            FROM Transactions tr
        ...
        var expenseByCategory = Db.Query(byCategorySql, BuildParams(context, from, to, typeId, categoryId, keyword));
        ...
        WebUtil.WriteJson(context, new {
            ok = true,
            period = period,
            summary = new { totalIncome = income, totalExpense = expense, balance = income - expense },
